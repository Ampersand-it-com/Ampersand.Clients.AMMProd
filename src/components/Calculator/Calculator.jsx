...
function Calculator() {
  ...
  const { videoSelectOptions, animationSelectOptions } =
    useTypeOfContentOptions();
  ...
  return (
    <div className={s.root} id="priceCalculator">
      <h2
        className={classNames(s.title, { [s.titleAnimation]: isTitleShown })}
        ref={titleRef}
      >
      ...
      <p
        className={classNames(s.description, {
          [s.descriptionAnimation]: isDescriptionlShown,
        })}
        ref={descriptionRef}
      >
      ...
      <div className={s.formContainer}>
        <form className={s.calculatorForm}>
          <div className={s.radioContainer}>
            <label className={s.radioLabel}>
              {t("home.calculator.video")}
              <input
                defaultChecked={typeOfContent === "video"}
                type="radio"
                name="contentType"
                value="video"
                className={s.typeOfContent}
                onClick={(e) => handleContentTypeClick(e)}
              />
              <span className={s.checkmark}></span>
              <p className={s.checkmarkInner}></p>
            </label>
            <label className={s.radioLabel}>
              {t("home.calculator.animation")}
              <input
                defaultChecked={typeOfContent === "animation"}
                type="radio"
                name="contentType"
                value="animation"
                className={s.typeOfContent}
          ...
          {typeOfContent === "video" && (
            <Select
              ...
              placeholder={t("home.calculator.formPlaceholderType")}
              onChange={(e) => handleSubTypeOfContentChange(e)}
            />
          )}
            ...
            subTypeOfContent !== subTypeOfContentConst.tiktokVideo && (
              <>
                ...
                <Select
                  ...
                  styles={customStyles}
                  ref={ref}
                  options={get(durationOptions, subTypeOfContent)}
                  placeholder={t("home.calculator.formPlaceholderDuration")}
                  value={price.value}
          ...
          {subTypeOfContent && price.value !== 0 && (
            <>
              ...
              <Select
                ...
                placeholder={t("home.calculator.formPlaceholderOptions")}
                value={factors}
          ...
          {price !== 0 && (
            <button
              ...
              type="button"
              onClick={handleCalculatedIfValid}
            >
              {t("home.calculator.btn")}
            </button>
          )}
        </form>
        <Image
        ...
        ></Image>
        {isCalculated && (
          <div className={s.calculatedBlock}>
            <div className={s.finalPrice}>
              ${" "}
              {finalPrice > 1000
                ? Math.round(finalPrice / 100) * 100
                : Math.round(finalPrice / 10) * 10}
            </div>
            <div className={s.disclaimerTitle}>
              {t("home.calculator.disclaimerTitle")}
            </div>
            <div className={s.videoOptionsContainer}>
              <p>
                <span className={s.videoOptionsTitle}>
                  {t("home.calculator.type")}:
                </span>
                <span className={s.videoOptionsValue}>{typeOfContent}</span>
              </p>
              <p>
                <span className={s.videoOptionsTitle}>
                  {t("home.calculator.form")}:
                </span>
                <span className={s.videoOptionsValue}>
                  {t(
                    `home.calculator.${
                      typeOfContent === "video"
                        ? "videoOption"
              ...
              <p>
                <span className={s.videoOptionsTitle}>
                  {t("home.calculator.options")}:
                </span>
                <span className={s.optionsValues}>
                  {factors.map((factor, i) => (
                    <span key={i}>{factor.label}</span>
                  ))}
                </span>
              </p>
            </div>
            <div className={s.finalpriceContainer}>
              <div className={s.finalPriceWrapper}>
                <div className={s.finalPriceDesctop}>
                  ${" "}
                  {finalPrice > 1000
                    ? Math.round(finalPrice / 100) * 100
                    : Math.round(finalPrice / 10) * 10}
                </div>
                <div className={s.disclaimerTitleDesctop}>
                  {t("home.calculator.disclaimerTitle")}
                </div>
              </div>
              <div className={s.videoDisclaimer}>
